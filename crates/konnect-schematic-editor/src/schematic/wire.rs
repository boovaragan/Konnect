...
impl Wire {
    ...
    pub fn is_horizontal(&self) -> bool { (self.start.1 - self.end.1).abs() < 1e-9 }
    pub fn is_vertical(&self)   -> bool { (self.start.0 - self.end.0).abs() < 1e-9 }
...
pub struct WireCollection { wires: Vec<Wire> }

impl WireCollection {
    pub fn new(wires: Vec<Wire>) -> Self { WireCollection { wires } }

    pub fn len(&self)      -> usize { self.wires.len() }
    pub fn is_empty(&self) -> bool  { self.wires.is_empty() }
    pub fn iter(&self)     -> std::slice::Iter<'_, Wire>     { self.wires.iter() }
    pub fn iter_mut(&mut self) -> std::slice::IterMut<'_, Wire> { self.wires.iter_mut() }
    pub fn get(&self, i: usize) -> Option<&Wire>             { self.wires.get(i) }
    pub fn get_mut(&mut self, i: usize) -> Option<&mut Wire>  { self.wires.get_mut(i) }
    pub fn push(&mut self, w: Wire)                          { self.wires.push(w); }
    pub fn as_slice(&self) -> &[Wire]                        { &self.wires }
    pub fn into_vec(self) -> Vec<Wire>                       { self.wires }
    ...
    pub fn within_rectangle(&self, x1: f64, y1: f64, x2: f64, y2: f64) -> Vec<&Wire> {
        ...
        self.wires.iter().filter(|w| in_r(w.start) || in_r(w.end)).collect()
    }
}

impl std::ops::Index<usize> for WireCollection {
    type Output = Wire;
    fn index(&self, i: usize) -> &Wire { &self.wires[i] }
}
impl<'a> IntoIterator for &'a WireCollection {
    type Item = &'a Wire;
    type IntoIter = std::slice::Iter<'a, Wire>;
    fn into_iter(self) -> Self::IntoIter { self.wires.iter() }
}
