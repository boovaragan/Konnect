...
impl Symbol {
    ...
    pub fn from_sexp(node: &SexpNode) -> Result<Self> {
        ...
        let mirror = node.find("mirror").and_then(|n| n.value()).map(str::to_owned);
        let unit: u32 = node.get_value("unit").and_then(|s| s.parse().ok()).unwrap_or(1);
        ...
        Ok(Symbol {
            lib_id, at, mirror, unit, in_bom, on_board, dnp,
            fields_autoplaced, uuid, properties, raw_sub_nodes,
        })
    }
    ...
    pub fn value_str(&self) -> Option<&str>  { self.property("Value") }
    pub fn footprint(&self) -> Option<&str>  { self.property("Footprint") }
    pub fn datasheet(&self) -> Option<&str>  { self.property("Datasheet") }

    pub fn set_reference(&mut self, v: &str) { self.set_property("Reference", v); }
    pub fn set_value_str(&mut self, v: &str) { self.set_property("Value", v); }
    pub fn set_footprint(&mut self, v: &str) { self.set_property("Footprint", v); }
    pub fn set_datasheet(&mut self, v: &str) { self.set_property("Datasheet", v); }
...
impl std::fmt::Display for Symbol {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "<Symbol {} ({})>", self.reference().unwrap_or("?"), self.lib_id)
    }
}
...
impl SymbolCollection {
    ...
    pub fn iter(&self)   -> std::slice::Iter<'_, Symbol>     { self.symbols.iter() }
    pub fn iter_mut(&mut self) -> std::slice::IterMut<'_, Symbol> { self.symbols.iter_mut() }
    pub fn get(&self, i: usize) -> Option<&Symbol>           { self.symbols.get(i) }
    pub fn get_mut(&mut self, i: usize) -> Option<&mut Symbol> { self.symbols.get_mut(i) }
    pub fn as_slice(&self) -> &[Symbol]                      { &self.symbols }
    pub fn push(&mut self, s: Symbol)                        { self.symbols.push(s); }
    pub fn into_vec(self) -> Vec<Symbol>                     { self.symbols }

    // mutation
    pub fn remove_by_reference(&mut self, reference: &str) -> Option<Symbol> {
        let idx = self.symbols.iter().position(|s| s.reference() == Some(reference))?;
    ...
    pub fn reference_startswith(&self, prefix: &str) -> Vec<&Symbol> {
        self.symbols.iter()
            .filter(|s| s.reference().map(|r| r.starts_with(prefix)).unwrap_or(false))
            .collect()
    }

    pub fn by_value(&self, value: &str) -> Vec<&Symbol> {
        self.symbols.iter().filter(|s| s.value_str() == Some(value)).collect()
    }

    pub fn value_startswith(&self, prefix: &str) -> Vec<&Symbol> {
        self.symbols.iter()
            .filter(|s| s.value_str().map(|v| v.starts_with(prefix)).unwrap_or(false))
            .collect()
    }
    ...
    pub fn set_all_dnp(&mut self, dnp: bool) {
        for s in &mut self.symbols {
            if s.reference().map(|r| r.starts_with('#')).unwrap_or(false) { continue; }
            s.dnp = dnp;
        }
    }
}
...
impl std::ops::Index<usize> for SymbolCollection {
    type Output = Symbol;
    fn index(&self, i: usize) -> &Symbol { &self.symbols[i] }
}
impl std::ops::IndexMut<usize> for SymbolCollection {
    fn index_mut(&mut self, i: usize) -> &mut Symbol { &mut self.symbols[i] }
}
...
impl<'a> IntoIterator for &'a mut SymbolCollection {
    ...
    fn into_iter(self) -> Self::IntoIter { self.symbols.iter_mut() }
}
